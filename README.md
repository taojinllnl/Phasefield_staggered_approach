...
4. Various phase-field models, including the AT-1 and AT-2 models for brittle fracture (Griffith), as well as several cohesive phase-field models for quasi-brittle fracture, are provided.
5. The techniques of the over-relaxation and the Anderson acceleration are provided to speed up the convergence of the staggered iterations.
...
### Recent Updates
1. Add the techniques of the over-relaxation and the Anderson acceleration are provided to speed up the convergence of the staggered iterations. To recover the naive staggered approach, simply set ```Over relaxation omega = 1.0``` and ```Anderson acceleration depth = 0``` in the ```parameters.prm``` file (Apr. 29th, 2026).
2. Add the option of AT-1 cohesive model (Apr. 10th, 2026).
3. Add the adaptive mesh refinement option, remove some unused items in the parameters.prm file (Feb. 14th, 2026).
4. Add the plane-stress option to the code and input files (Feb. 15th, 2026).
5. Add the option of AT-2 (default), AT-1, and phase-field cohesive zone model (PFCZM). In the parameters.prm file, use the flag "Phase-field model type" to choose the phase-field model. Also, in the materialDataFile, extra material data entries need to be provided for the PFCZM model (Feb. 24th, 2026).

### How to compile
The finite element procedure for the phase-field staggered approach is implemented in [deal.II](https://www.dealii.org/) (with the develop branch as Aug. 17th, 2024), which is an open source finite element library. In order to use the code (**main.cc**) provided here, deal.II should be configured with MPI and at least with the interfaces to BLAS, LAPACK, Threading Building Blocks (TBB), and UMFPACK. For optional interfaces to other software packages, see https://www.dealii.org/developer/readme.html.
